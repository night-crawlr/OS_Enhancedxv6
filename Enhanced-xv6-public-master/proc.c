...
struct proc *queue[5][NPROC];      // each q[i] is similar to page table
int cur_size[5] = {0, 0, 0, 0, 0}; // cur no of process in q[i] is cur_size[i]
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
...
#ifdef PBS
      struct proc *high_p = p;
      high_p = priority_scheduler(p); // return high priority process from ptable
      p = high_p;
      ...
      c->proc = p;
      p->w_time = 0; // making w_time =0 when cpu picks
      switchuvm(p);
      p->state = RUNNING;
      p->n_run++;
...
int myps(void)
{
  ...
  sti(); //Enable interupts on this processor

  acquire(&ptable.lock);
  //cprintf("  PID    Name    Priority    State     r_time    w_time    n_run    cur_q    q0    q1    q2    q3    q4 \n\n");
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
#ifdef MLFQ
    p->priority = -1;
    ...
    if (p->state == RUNNABLE)
      cprintf(" PID=%d    Name=%s    Priority = %d    State = Runnable    r_time = %d    w_time = %d    n_run = %d    cur_q = %d    q0 = %d    q1 = %d    q2 = %d    q3 = %d    q4 = %d\n\n", p->pid, p->name, p->priority, p->rtime, p->w_time, p->n_run, p->cur_q, p->total_r_time_q[0], p->total_r_time_q[1], p->total_r_time_q[2], p->total_r_time_q[3], p->total_r_time_q[4]);
    if (p->state == RUNNING)
      cprintf(" PID=%d    Name=%s    Priority = %d    State = Running     r_time = %d    w_time = %d    n_run = %d    cur_q = %d    q0 = %d    q1 = %d    q2 = %d    q3 = %d    q4 = %d\n\n", p->pid, p->name, p->priority, p->rtime, p->w_time, p->n_run, p->cur_q, p->total_r_time_q[0], p->total_r_time_q[1], p->total_r_time_q[2], p->total_r_time_q[3], p->total_r_time_q[4]);
    if (p->state == SLEEPING)
      cprintf(" PID=%d    Name=%s    Priority = %d    State = Sleeping    r_time = %d    w_time = %d    n_run = %d    cur_q = %d    q0 = %d    q1 = %d    q2 = %d    q3 = %d    q4 = %d\n\n", p->pid, p->name, p->priority, p->rtime, p->w_time, p->n_run, p->cur_q, p->total_r_time_q[0], p->total_r_time_q[1], p->total_r_time_q[2], p->total_r_time_q[3], p->total_r_time_q[4]);
    if (p->state == ZOMBIE)
      cprintf(" PID=%d    Name=%s    Priority = %d    State = Zombie      r_time = %d    w_time = %d    n_run = %d    cur_q = %d    q0 = %d    q1 = %d    q2 = %d    q3 = %d    q4 = %d\n\n", p->pid, p->name, p->priority, p->rtime, p->w_time, p->n_run, p->cur_q, p->total_r_time_q[0], p->total_r_time_q[1], p->total_r_time_q[2], p->total_r_time_q[3], p->total_r_time_q[4]);
  }
  release(&ptable.lock);
  return 22;
}
...
void update_all_times_process(void)
{
  acquire(&ptable.lock);
  struct proc *p;
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
